...
const KatakanaAlphabet = () => {
  ...
  const handleSelectVocabulary = (item) => {
    dispatch(addVocabulary({ newVocabulary: item, scriptType: "katakana" }));
  };

  const handleModalButtonClick = () => {
    if (
      !katakanaVocabularyList.some(
        (vocab) => vocab.id === selectedKatakanaItem.id
      )
    ) {
      dispatch(
        addVocabulary({
          newVocabulary: selectedKatakanaItem,
          scriptType: "katakana",
        })
      );
    } else {
